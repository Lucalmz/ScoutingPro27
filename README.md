...
- [二、 使用说明 (User Guide)](#二-使用说明-user-guide)
  ...
  - [2.6 第五步：比赛现场侦察——单手急速盲操打分 (Match Scouting)](#26-第五步比赛现场侦察单手急速盲操打分-match-scouting)
  - [2.7 第六步：战力天梯榜与“吹牛指数”量化对账 (Brag Index)](#27-第六步战力天梯榜与吹牛指数量化对账-brag-index)
  - [2.8 第七步：战术 AI 军师助手](#28-第七步战术-ai-军师助手)
  - [2.9 第八步：数据导出与 U 盘离线应急同步包](#29-第八步数据导出与-u-盘离线应急同步包)
...
2. 输入要走访的队伍编号（例如 `#27570`），勾选他们的真实硬件参数：
   ...
   - **悬挂攀爬形式 (Hang)**：卷扬绳索攀爬 (Winch)、级联滑轨抬升 (Slide)、连杆机械锁定 (Linkage)、被动卡扣勾挂 (Passive)；
...
3. **特写拍照**：点击拍照按钮，拍下对方机器人关键部位的高清特写照片。手机会自动对照片进行 WebP 高保真优化压缩并保存。
...
### 2.6 第五步：比赛现场侦察——单手急速盲操打分 (Match Scouting)
1. 坐在看台上，比赛开始！队员打开 **【现场侦察】** 表单。
2. 支持**单队记录**与**联盟双队记录**灵活切换。界面专为现场单手急速盲操设计：
   - **自动阶段 (Autonomous)**：大号加减按钮快速记录移动停靠得分 (Movement)、有效图案得分 (Patterns, 2分)、分类得分 (Classified, 3分) 与溢出得分 (Overflow, 1分)；
   - **手动阶段 (Teleop)**：快速记录触发大门闸门次数 (Gates Triggered)、分类得分 (Classified, 3分) 与溢出得分 (Overflow, 1分)；
   - **终局阶段 (Endgame)**：勾选基础驻留得分 (Base Score: 5分 / 10分)、支撑联盟 1+1 悬挂状态 (+18分)；
   - **故障快速标记**：如果机器人上场意外掉线、断电或机械卡死，直接点亮红色的 **【车坏了 (Is Broken)】** 警报；
   - **战术标签与备忘录**：现场快速为该战队打上战术标签（如高速吸取、防守凶悍、易侧翻等），并输入打法备注。
3. 比赛哨响，点击 **【提交记录】**，数据 0.05 秒直飞领队电脑！
4. **如果赛场突然完全断网怎么办？**  
   **完全不用慌！** 系统会自动开启离线保护，数据稳妥存入手机本地；一旦网络恢复，后台会自动将离线记录一条不差地毫秒级回传，零丢单！
